...
function coax_dicom_convert(file_format,dir_in,dir_out,varargin)
...
%
% coax_dicom_convert(file_format,data_directory,output_directory);
%
...
if nargin<2 | isempty(dir_in)
    dir_in = uigetdir(current_dir,'PLEASE SELECT PARENT DATA DIRECTORY...');
end
...
for a=1:length(scan_info)
    ...
    if scan_info(a).isdir && (~strcmpi(scan_info(a).name,'.') && ~strcmpi(scan_info(a).name,'..')) %&& ~isnan(str2double(scan_info(a).name))
        ...
        scan_names  = deblank(cat(1,scan_names,{scan_info(a).name,protocol}));
...
while ~name_flag
    ...
    %scan_names(:,3) = inputdlg(prompt,'RENAME SCANS IF DESIRED...',1,scan_names(:,2)');
    scan_names(:,3) = scan_names(:,2);
...
for a=1:size(scan_names,1)
    ...
    if ~exist(export_dir,'dir')
        %eval(sprintf('mkdir %s;',export_dir));
        mkdir(export_dir);
    end
    ...
    coax_dicom_to_nifti(hdr,opts,root_dir,format);
